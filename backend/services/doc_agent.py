...
def _sync_ocr_image_worker(file_path: str) -> dict:
    """Worker core executing blocking OCR on thread engine with proper error handling."""
    import os
    try:
        ...
        if not pytesseract.pytesseract.tesseract_cmd:
            return {
                "_error": "Tesseract is not installed or not available in PATH. OCR skipped; upload was still stored safely.",
        ...
        return {"_ocr_empty": True, "_warning": f"No text extracted from {Path(file_path).name}"}

    except ImportError as e:
        return {"_error": f"pytesseract or PIL not installed: {str(e)}", "_extraction_failed": True}
    except FileNotFoundError:
        return {"_error": "Tesseract not found in PATH or configured paths. Install Tesseract-OCR to enable OCR.", "_extraction_failed": True}
    except Exception as e:
        return {"_error": f"OCR processing failed: {str(e)}", "_extraction_failed": True}
